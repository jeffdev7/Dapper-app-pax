...
namespace Dapper.menu.Domain.Persistence.Repositories
{
        ...
        where TEntity : BaseEntity
    {
        ...
        public async Task<IEnumerable<TEntity>> GetListPaged(int pageNumber, int rowPerPages, 
            string conditions, string orderby)
        {
            await using var connection = new SqlConnection(ConnectionString);
            var result = await connection.GetListPagedAsync<TEntity>(pageNumber, rowPerPages, 
                conditions, orderby);
            await connection.CloseAsync();
            return result;
        }
        ...
        public async Task<IEnumerable<TEntity>> Execute(string query, object? parameters = null, 
            CommandType commandType = CommandType.Text, int commandTimeOut = 60)
        {
            await using var connection = new SqlConnection(ConnectionString);
        ...
        public async Task<T> ExecuteSingle<T>(string query, object? parameters = null, 
            CommandType commandType = CommandType.Text, int commandTimeOut = 60)
        {
            await using var connection = new SqlConnection(ConnectionString);
            var result = connection.QuerySingle<T>(query, parameters, commandType: commandType, 
        ...
        {
            await using var connection = new SqlConnection(ConnectionString);
            var result = connection.QuerySingleOrDefault<TEntity>(query, parameters, 
